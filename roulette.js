    ...
    for (let i = 0; i < totalTickets; i++) {
        ...
        ticket.innerHTML = `
            <div class="ticket-content">
                ...
                <span class="amount">${prize.amount} Tether</span>
            </div>
        `;
        ticket.style.top = `${i * 120 - 60}px`;
...
function spin() {
    ...
    const finalPosition = randomIndex * ticketHeight + 50; // Center position
...
// Navigation buttons (for visual effect)
function scrollRoulette(direction) {
    if (isSpinning) return;
    
    const roulette = document.getElementById('roulette');
    const currentTransform = roulette.style.transform || 'translateY(0)';
    const currentY = parseInt(currentTransform.match(/-?\d+/) || [0])[0];
    const scrollAmount = 120;
    const newY = direction === 'left' ? currentY - scrollAmount : currentY + scrollAmount;
    
    roulette.style.transition = 'transform 0.5s ease';
    roulette.style.transform = `translateY(${newY}px)`;
...
document.getElementById('leftBtn').addEventListener('click', () => scrollRoulette('left'));
document.getElementById('rightBtn').addEventListener('click', () => scrollRoulette('right'));
